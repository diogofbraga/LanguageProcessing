...
\begin{itemize}
	...
	\item Funções de Transição:
		\begin{itemize}
			\item INIT $\rightarrow$ $<page>$ $\rightarrow$ PAGE
			\item INIT $\rightarrow$ $</page>$ $\rightarrow$ INIT
			\item PAGE $\rightarrow$ $<title>$Proverbios.*$</title>$ $\rightarrow$ PAGE\_CONTEUDO
			\item PAGE\_CONTEUDO $\rightarrow$ $*[\ ]*\&quot;[\ ]*$ $\rightarrow$ QUOTE
			\item QUOTE $\rightarrow$ .* $*[\ ]*\&quot;[\ ]*$ $\rightarrow$	QUOTE\_CONT
			\item QUOTE\_CONT $\rightarrow$ $*[\ ]*\&quot;[\ ]*$ $\rightarrow$ QUOTE
			\item QUOTE\_CONT $\rightarrow$ $**\{adulterados\}$ $\rightarrow$ QUOTES\_ADULTERADOS
			\item QUOTE\_CONT $\rightarrow$ $**\{adulteracao\}$ $\rightarrow$ QUOTES\_ADULTERADOS
			\item QUOTE\_CONT $\rightarrow$ $**\{alternativos\}$ $\rightarrow$ QUOTES\_ALTERNATIVOS
		\end{itemize}
\end{itemize}
